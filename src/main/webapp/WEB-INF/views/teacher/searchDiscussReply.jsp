            ...
            <table class="table table-striped table-hover" style="table-layout:fixed;">
                ...
                <c:forEach items="${pageInfo.list}" var="post">
                    <tr>
                        ...
                        <td>${post.content}</td>
                        <c:if test="${post.student.name!=null}">
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination">
                    <c:if test="${pageInfo.hasPreviousPage }">
                        <li>
                            <a href="${APP_PATH}/admin/searchDiscussReply/${id}?pageNum=1&pageNumber=${pageNumber}">首页</a>
                        </li>
                        <li>
                            <a href="${APP_PATH}/admin/searchDiscussReply/${id}?pageNum=${pageInfo.pageNum-1}&pageNumber=${pageNumber}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                            <li>
                                <a href="${APP_PATH }/admin/searchDiscussReply/${id}?pageNum=${page_Num }&pageNumber=${pageNumber}">${page_Num }</a>
                            </li>
                        </c:if>
                    </c:forEach>
                    ...
                    <c:if test="${pageInfo.hasNextPage }">
                        ...
                        <li>
                            <a href="${APP_PATH }/admin/searchDiscussReply/${id}?pageNum=${pageInfo.pages}&pageNumber=${pageNumber}">末页</a>
                        </li>
                    </c:if>
                    <c:if test="${!pageInfo.hasNextPage}">
                        <li><a href="javascript:void(0)" style="pointer-events: none"
...
<script type="text/javascript">
    ...
    $(".toPage").on("click", function () {
        ...
        window.location.href = "${APP_PATH}/admin/searchDiscussReply/${id}?pageNumber=${pageNumber}&pageNum=" + pageNum;
    });

    function back() {
        window.location.href = "${APP_PATH}/admin/searchDiscussInfo?pageNum=${pageNumber}";
    }
    ...
    $("#search").on("click", function () {
        ...
        //发送ajax请求
        $.ajax({
            url: "${APP_PATH}/admin/searchDiscussPostByTerm",
            type: "POST",
            dataType: "json",
            data: {
                "name": name,
            ...
            success: function (result) {
                ...
                if (result.code === 100) {
                    ...
                    window.location.href = "${APP_PATH}/admin/searchDiscussReply/${id}?pageNumber=${pageNumber}";
                }
            },
            error: function () {
    ...
    $("#post_batchDelete_btn").on("click", function () {
        ...
        //发送ajax请求删除
        $.ajax({
            url: "${APP_PATH}/admin/deleteDiscussPostBatch",
            type: "POST",
            dataType: "json",
            data: {
                "ids": ids
            ...
            success: function (result) {
                ...
                if (result.code === 100) {
                    ...
                    window.location.href = "${APP_PATH}/admin/searchDiscussReply/${id}?pageNumber=${pageNumber}&pageNum=" + ${pageInfo.pageNum};
                }
            },
            error: function () {
                layer.msg("网络异常，请稍后再试", {time: 1500, icon: 5, shift: 6}, function () {
