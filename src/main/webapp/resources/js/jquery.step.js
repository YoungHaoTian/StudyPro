(function (factory) {
    ...
    (function ($) {
        $.fn.step = function (options) {
            let opts = $.extend({}, $.fn.step.defaults, options);
            let size = this.find(".step-header li").length;
            let barWidth = opts.initStep < size ? 100 / (2 * size) + 100 * (opts.initStep - 1) / size : 100;
